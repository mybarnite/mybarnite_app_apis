...
function __construct()
					...
					{	
                        ...
                        if($order_id === NULL){
                            $data = array(
                                ...
                                'bar_booking_start_date'=>date("Y-m-d", strtotime($bookingDate)),
	...
	function checkAvailability_post()
	{
		try{
			...
			if($hallBooking==1)			
			{
				$sql1 = 'select hall_fee, seat_for_basic as seats from bars_list where id='.$barId;
				...
				else
				{
					if($num_rows > 0)
					{
						...
						$bar_hall_fee= $row['hall_fee'];
						$hall_capacity = $row['seats'];
					}	
				}
				if($orderId!="")
				{
				...
				else
				{
					...
					$bar_hall_fee = ($num_rows > 0)?0:$bar_hall_fee;
					...
					$data["total_amount"]=$bar_hall_fee;
			...
			{
				$sql1 = 'select seat_for_basic as seats, cost_per_seat from bars_list where id='.$barId;
				$query1 = $this->db->query($sql1);
				$num_rows=$query1->num_rows();
				if (!$query1)
				{
				...
				{
					$sql = 'SELECT SUM(no_of_persons) as seats FROM tbl_order_history WHERE bar_id='.$barId.' and payment_status = "Done" and no_of_persons >0 and bar_booking_start_date = "'.Date("Y-m-d",strtotime($startingTime)).'" AND bar_booking_starts >=  "'.$startingTime.'" AND bar_booking_ends <=  "'.$endingTime.'"';
				...
				else
				{
					if($num_rows1 > 0)
					{
						...
						$data["total_amount"]=$totalCostForBookedSeats;
						$this->set_response([
							'status' =>'SUCCESS',
							'data' => $data
						], REST_Controller::HTTP_OK); 	
